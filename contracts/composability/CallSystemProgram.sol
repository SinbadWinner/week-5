...
contract CallSystemProgram {
    ...
    (
        bytes32[] memory accounts1,
        bool[] memory isSigner1,
        bool[] memory isWritable1,
        bytes memory data1,
        uint64 rentExemption
    ) = LibSystemProgram.formatCreateAccountWithSeedInstruction(
        payer,
        basePubKey,
        programId,
    ...
    // Transfer
    (
        bytes32[] memory accounts2,
        bool[] memory isSigner2,
        bool[] memory isWritable2,
        bytes memory data2
    ) = LibSystemProgram.formatTransferInstruction(
        payer,
        recipient,
    ...
    CALL_SOLANA.execute(rentExemption + amount, createIx);
    CALL_SOLANA.execute(amount, transferIx);
}
}